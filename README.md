...
　・BGMの選択について、一つのDBでログインしている人とログインしていない人を区別していることです。  
　　（DBにログインしている人はアソシエーションしているログインIDを紐付けさせています）  
　　また、ログインしていない人が複数同時に利用しても各々が選択した曲が流れるようしています。  
　　（常にDBの一番最後に登録された曲が即時流れる曲としています。ただし、この方式ではアクセス者数が  
　　増えるとDBを圧迫する可能性があると判断したので最初にアクセスした時点で未ログイン者のDBは  
　　全て削除して、初期値としてActiveHashの一番上の曲をセットし直すようにしました）  
　　この対応を行うことでDBの取り扱いについて大変良い自己学習が出来ました。

　・BGMを流す方法については、様々なサイトの情報を組みあせて作成した。  
　　結果的にはaudio要素で音楽自体は鳴らし、これをユーザー側でコントロールするためにJavascriptを利用し、  
　　選択した曲はDBに保存するためrailsのMVCで取り扱う形にしました。  
