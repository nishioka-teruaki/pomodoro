...
　　また、ログインしている人・ログインしていない人が複数同時に利用しても各々が選択した曲が流れるようしています。  
　　（常にログインIDの中でDBの一番最後に登録された曲が即時流れるようにしております。  
　　ただし、この方式ではアクセス者数が増えるとDBを圧迫する可能性があると判断したので、ログインしているユーザーは  
　　新しい曲を選択する度に古いDBのレコードを削除、ログインしていないユーザーは当サイトに最初にアクセスしたときに  
　　DBを削除して、初期値としてActiveHashの一番上の曲をセットし直すようにしました）  
　　この対応を行いDBの取り扱いとMVCモデルの取り扱いについて大変良い自己学習が出来ました。  

　・BGMを流す方法については、取り扱っているサイトが少なく様々なサイトの情報を組みあせて作成しました。  
　　結果的にはaudio要素で音楽自体は鳴らし、  
　　これをユーザー側でコントロールするためにJavascriptをすることとしました。  
